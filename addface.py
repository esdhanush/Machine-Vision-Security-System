...
detected_persons = {}
def save_known_faces_to_json():
    known_faces_data = {
        "known_face_encodings": [enc.tolist() for enc in known_face_encodings],
        "known_face_names": known_face_names
    }

    with open("known_faces_data.json", "w") as json_file:
        json.dump(known_faces_data, json_file)

# Function to load known faces from JSON
def load_known_faces_from_json():
    global known_face_encodings, known_face_names

    if os.path.exists("known_faces_data.json"):
        with open("known_faces_data.json", "r") as json_file:
            known_faces_data = json.load(json_file)
            known_face_encodings = [np.array(enc) for enc in known_faces_data["known_face_encodings"]]
            known_face_names = known_faces_data["known_face_names"]

load_known_faces_from_json()  # Load known faces from JSON
...
# Initialize variables for displaying last detected photos
...
def add_new_person():
    ...
    print(f"Capturing images for {new_person_name}. Press 'q' to finish.")
    capturing = True
    ...
    while capturing:
        ...
        cv2.imshow('Capture Images', frame)  # Display the frame

        key = cv2.waitKey(1) & 0xFF
        if key == ord('q'):  # Press 'q' to finish capturing
            capturing = False

    # Check if at least one face was captured
    if len(new_person_face_encodings) > 0:
        # Calculate the average face encoding
        average_face_encoding = np.mean(new_person_face_encodings, axis=0)

        # Add the new person to the known faces
        known_face_encodings.append(average_face_encoding)
        known_face_names.append(new_person_name)

        print(f"{new_person_name} added as a known person.")

        # Save the updated known faces
        save_known_faces()

        # Restart the face recognition model with the updated known faces
        initialize_face_recognition_model()
...
def initialize_face_recognition_model():
    ...
    # Load known faces from the saved data
    if os.path.exists("known_faces_data.json"):
        with open("known_faces_data.json", "r") as json_file:
            known_faces_data = json.load(json_file)
            known_face_encodings = [np.array(enc) for enc in known_faces_data["known_face_encodings"]]
...
while True:
    ...
    if key == ord('a'):  # Press 'a' to add a new person
        add_new_person()
