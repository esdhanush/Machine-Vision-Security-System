...
# Initialize a unique ID counter for unknown persons
unique_person_id = 0
...
# Directory to store detected but unrecognized persons
UNRECOGNIZED_PERSONS_DIR = "unrecognized_persons"
if not os.path.exists(UNRECOGNIZED_PERSONS_DIR):
    os.makedirs(UNRECOGNIZED_PERSONS_DIR)

# Dictionary to store detected persons' information
detected_persons = {}

def assign_unique_person_id():
    global unique_person_id
    unique_person_id += 1
    return f"Person{unique_person_id}"
...
while True:
    ...
    for face_location, face_encoding in zip(face_locations, face_encodings):
        ...
        # Assign a unique person ID if the face is unknown
        if name == "Unknown Person":
            unique_id = assign_unique_person_id()
            name = unique_id
            cv2.putText(frame, name, (left, bottom + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 255, 0), 2)

            # Save the detected but unrecognized person's frame
            timestamp = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            filename = f"{unique_id}_{timestamp.replace(':', '-')}.jpg"
            filepath = os.path.join(UNRECOGNIZED_PERSONS_DIR, filename)
            cv2.imwrite(filepath, frame)
        else:
            # Display the recognized name for trained individuals
            cv2.putText(frame, name, (left, bottom + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 255, 0), 2)
            # Check if the recognized person is in the detected_persons dictionary
            if name in detected_persons:
                # Display the last detected timestamp for known individuals
                last_detected_time = detected_persons[name][-1]
                cv2.putText(frame, last_detected_time, (left, bottom + 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0),
                            1)

        # Continue tracking the person with that ID until they show their face and are recognized
